...
#define IF_FALSE_PRINT_RETURN(condition, message) \
    if (!(condition)) {                           \
        std::cout << "File" << __FILE__ << " Line " << __LINE__ << ": " << message << std::endl; \
        return false;                             \
    }                                             \
    ...
    bool test_multiple_pools(const GeneralHeContext& context, size_t device_index, size_t thread_index = 0) {
        ...
        auto good_pool = [context_pool](MemoryPoolHandle pool, MemoryPoolHandle expect) {
            if (context_pool == nullptr && expect == nullptr && pool == nullptr) {
                return true;
            }
            return pool != context_pool && pool == expect;
        };
        ...
        if (scheme == SchemeType::BFV || scheme == SchemeType::BGV) { 
            ...
            { // encode_polynomial
                ...
                GeneralVector decoded = context.encoder().decode_polynomial(encoded);
            ...
            if (scheme == SchemeType::BFV) {
                ...
                { // scale_up and scale_down
                    ...
                    GeneralVector decoded = context.encoder().decode_polynomial(scaled_down);
                ...
                { // scale_up_inplace and scale_down_inplace
                    ...
                    GeneralVector decoded = context.encoder().decode_polynomial(scaled);
        ...
        { // test encryption and decryption
            ...
            { // encrypt_zero_asymmetric
                ...
                GeneralVector result = context.encoder().decode_polynomial(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(zeros, result), "encrypt_zero_asymmetric/correct");
            }
            ...
            { // encrypt_zero_asymmetric_new
                ...
                Plaintext decrypted = context.decryptor().decrypt_new(encrypted, pool);
                GeneralVector result = context.encoder().decode_polynomial(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(zeros, result), "encrypt_zero_asymmetric_new/correct");
            }
            ...
            { // encrypt_zero_symmetric
                ...
                Plaintext decrypted = context.decryptor().decrypt_new(encrypted, pool);
                GeneralVector result = context.encoder().decode_polynomial(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(zeros, result), "encrypt_zero_symmetric/correct");
            }
            ...
            { // encrypt_zero_symmetric_new
                ...
                Plaintext decrypted = context.decryptor().decrypt_new(encrypted, pool);
                GeneralVector result = context.encoder().decode_polynomial(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(zeros, result), "encrypt_zero_symmetric_new/correct");
            }
        ...
        { // multiply
            ...
            { // multiply_inplace
                Ciphertext encrypted0 = context.encryptor().encrypt_asymmetric_new(encoded0, nullptr, pool);
                Ciphertext encrypted1 = context.encryptor().encrypt_asymmetric_new(encoded1, nullptr, pool);
                context.evaluator().multiply_inplace(encrypted0, encrypted1);
        ...
        { // square
            ...
            { // add_inplace
                Ciphertext encrypted = context.encryptor().encrypt_asymmetric_new(encoded, nullptr, pool);
                context.evaluator().square_inplace(encrypted);
        ...
        { // keyswitching
            ...
            KeyGenerator keygen_other = KeyGenerator(context.context(), context_pool);
            ...
            KSwitchKeys kswitch_key = context.key_generator().create_keyswitching_key(secret_key_other, false, context_pool);
            context_pool->deny();

            MemoryPoolHandle pool = MemoryPool::create(device_index);
            GeneralVector message = context.random_simd_full();
            Plaintext encoded = context.encoder().encode_simd(message, std::nullopt, scale, pool);
            Ciphertext encrypted_original = context.encryptor().encrypt_asymmetric_new(encoded, nullptr, pool);
        ...
        { // relinearize
            ...
            { // relinearize
                ...
                GeneralVector decoded = context.encoder().decode_simd(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(message, decoded), "relinearize/correct");
            }
            { // relinearize_inplace
                ...
                Plaintext decrypted = context.decryptor().decrypt_new(relinearized, pool);
                GeneralVector decoded = context.encoder().decode_simd(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(message, decoded), "relinearize_inplace/correct");
            }
            { // relinearize_new
                ...
                Plaintext decrypted = context.decryptor().decrypt_new(relinearized, pool);
                GeneralVector decoded = context.encoder().decode_simd(decrypted);
                IF_FALSE_PRINT_RETURN(context.near_equal(message, decoded), "relinearize_new/correct");
            }
        }
        ...
        if (scheme == SchemeType::CKKS) { // mod_switch_plain_to_next
            ...
            { // mod_switch_plain_to_next
                ...
                GeneralVector decoded = context.encoder().decode_simd(mod_switched);
        ...
        // TODO: test extract_lwe, assemble_lwe and pack_lwe
    ...
    void test_multi_devices(size_t threads, size_t device_count, bool device, SchemeType scheme, size_t n, size_t log_t, vector<size_t> log_qi, 
        ...
        auto test_thread = [=](int thread) {
            ...
            MemoryPoolHandle context_pool = MemoryPool::create(device_index);
            GeneralHeContext context(
                device, scheme, n, log_t, log_qi, 
                expand_mod_chain, seed, input_max, scale, tolerance, 
                to_device_after_keygeneration, use_special_prime_for_encryption, 
            ...
            MemoryPool::GlobalPool()->deny(true);
