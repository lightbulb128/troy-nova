...
namespace rns_tool {
    ...
    bool test_body_divide_and_round_q_last_ntt_inplace(bool device) {
        
        size_t poly_modulus_degree = 2;
        Modulus plain_t(0);
        RNSTool rns_tool(poly_modulus_degree, to_rns_base({53, 13}), plain_t);
        ...
        if (device) {
            ...
            ntt[1].to_device_inplace();
            ntt.to_device_inplace();
        }
        size_t base_q_size = rns_tool.base_q().size();
    ...
    TEST(RNSToolTest, HostDivideAndRoundQLastNTTInplace) {
        ASSERT_TRUE(test_body_divide_and_round_q_last_ntt_inplace(false));
    }
