...
namespace basics {
    ...
    TEST(Basics, HostBits) {
...

        {
            ...
            ConstSlice<uint64_t> sl(sl_data, 2, device);
        ...
        
        {
            ...
            ConstSlice<uint64_t> sl(sl_data, 5, device);
    ...
    bool test_add_uint128(ConstSlice<uint64_t> o1, ConstSlice<uint64_t> o2, ConstSlice<uint64_t> o3, uint8_t carry) {
        uint64_t result[2];
        Slice<uint64_t> result_slice(result, 2, on_device());
    ...
    __global__ void kernel_add(Slice<bool> test_result) {
        ...
        uint64_t o1[2]; ConstSlice<uint64_t> o1_slice(o1, 2, device);
        uint64_t o2[2]; ConstSlice<uint64_t> o2_slice(o2, 2, device);
        uint64_t o3[2]; ConstSlice<uint64_t> o3_slice(o3, 2, device);
    ...
    bool test_multiply_uint_uint64(ConstSlice<uint64_t> x, uint64_t y, size_t rlen, ConstSlice<uint64_t> r) {
        Array<uint64_t> result(rlen, on_device());
    ...
    TEST(Basics, HostMultiply) {
    ...
    
        {
            ...
            size_t r = 0;
            uint64_t result[0];
            EXPECT_TRUE(test_multiply_uint_uint64(ConstSlice<uint64_t>(x, 0, device), y, r, ConstSlice<uint64_t>(result, 0, device)));
        }
...

        {
            ...
            size_t r = 1;
            uint64_t result[1]{0};
            EXPECT_TRUE(test_multiply_uint_uint64(ConstSlice<uint64_t>(x, 0, device), y, r, ConstSlice<uint64_t>(result, 1, device)));
        }

        {
            ...
            EXPECT_TRUE(test_multiply_uint_uint64(ConstSlice<uint64_t>(x, 1, device), y, r, ConstSlice<uint64_t>(result, 1, device)));
        }

        {
            ...
            EXPECT_TRUE(test_multiply_uint_uint64(ConstSlice<uint64_t>(x, 1, device), y, r, ConstSlice<uint64_t>(result, 2, device)));
        }

        {
            ...
            EXPECT_TRUE(test_multiply_uint_uint64(ConstSlice<uint64_t>(x, 2, device), y, r, ConstSlice<uint64_t>(result, 3, device)));
        }

        {
            ...
            size_t r = 0;
            uint64_t result[0];
            EXPECT_TRUE(test_multiply_uint(ConstSlice<uint64_t>(x, 0, device), ConstSlice<uint64_t>(y, 0, device), r, ConstSlice<uint64_t>(result, 0, device)));
        }

        {
            ...
            size_t r = 1;
            uint64_t result[1]{0};
            EXPECT_TRUE(test_multiply_uint(ConstSlice<uint64_t>(x, 0, device), ConstSlice<uint64_t>(y, 1, device), r, ConstSlice<uint64_t>(result, 1, device)));
        }

        {
            ...
            EXPECT_TRUE(test_multiply_uint(ConstSlice<uint64_t>(x, 3, device), ConstSlice<uint64_t>(y, 3, device), r, ConstSlice<uint64_t>(result, 3, device)));
        }

        {
            ...
            EXPECT_TRUE(test_multiply_uint(ConstSlice<uint64_t>(x, 3, device), ConstSlice<uint64_t>(y, 3, device), r, ConstSlice<uint64_t>(result, 6, device)));
        }
    }
    ...
    bool test_divide_uint_inplace(ConstSlice<uint64_t> numerator, ConstSlice<uint64_t> denominator, ConstSlice<uint64_t> quotient, ConstSlice<uint64_t> remainder) {
        Array<uint64_t> n(numerator.size(), on_device());
        n.copy_from_slice(numerator);
        Array<uint64_t> d(denominator.size(), on_device());
        d.copy_from_slice(denominator);
        Array<uint64_t> q(numerator.size(), on_device());
    ...
    TEST(Basics, HostDivide) {
...

        {
            ...
            EXPECT_TRUE(test_divide_uint_inplace(ConstSlice<uint64_t>(numerator, 1, device), ConstSlice<uint64_t>(denominator, 1, device), ConstSlice<uint64_t>(quotient, 1, device), ConstSlice<uint64_t>(remainder, 1, device)));
        }
...

        {
            ...
            EXPECT_TRUE(test_divide_uint_inplace(ConstSlice<uint64_t>(numerator, 2, device), ConstSlice<uint64_t>(denominator, 2, device), ConstSlice<uint64_t>(quotient, 2, device), ConstSlice<uint64_t>(remainder, 1, device)));
        }

        {
            ...
            EXPECT_TRUE(test_divide_uint_inplace(ConstSlice<uint64_t>(numerator, 4, device), ConstSlice<uint64_t>(denominator, 2, device), ConstSlice<uint64_t>(quotient, 3, device), ConstSlice<uint64_t>(remainder, 2, device)));
        }
        
        {
            ...
            EXPECT_TRUE(test_divide_uint_inplace(ConstSlice<uint64_t>(numerator, 1, device), ConstSlice<uint64_t>(denominator, 2, device), ConstSlice<uint64_t>(quotient, 0, device), ConstSlice<uint64_t>(remainder, 1, device)));
        }
    }
