...
std::vector<Modulus> get_modulus_array(utils::ConstSlice<Modulus> modulus) {
    utils::Array<Modulus> array(modulus.size(), false);
    array.copy_from_slice(modulus);
...
void example_levels()
{
    ...
    */
    ...
    cout << hex;
    for (const auto &prime : get_modulus_array(context_data->parms().coeff_modulus()))
    {
        cout << prime.value() << " ";
    }
    cout << dec << endl;
    ...
    */
    context_data = context->first_context_data().value();
    while (true)
    {
        cout << " Level (chain index): " << context_data->chain_index();
        ...
        */
        if (!context_data->next_context_data().has_value()) break;
        context_data = context_data->next_context_data().value();
    } 
    ...
    */
    Plaintext plain;
    encoder.encode_polynomial({4, 3, 2, 1}, plain);
    ...
    */
    ...
    context_data = context->first_context_data().value();
    cout << "---->";
    while (context_data->next_context_data().has_value())
    {
    ...
    */
    ...
    cout << "Print the modulus switching chain." << endl;
    cout << "---->";
    for (context_data = context->key_context_data().value(); context_data->next_context_data().has_value(); context_data = context_data->next_context_data().value())
    {
        cout << " Level (chain index): " << context_data->chain_index() << endl;
        cout << "      parms_id: " << context_data->parms_id() << endl;
        cout << "      coeff_modulus primes: ";
