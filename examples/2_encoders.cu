...
void example_batch_encoder()
{
    ...
    simply changing `SchemeType::BFV` into `SchemeType::BGV` can make this example
    ...
    */
    auto qualifiers = context->first_context_data().value()->qualifiers();
...
void example_ckks_encoder()
{
    ...
    */
    ...
    // The nullopt indicates the encoding is done on the first level of the modulus chain.
    encoder.encode_complex64_simd(input, std::nullopt, scale, plain);
    ...
    */
    ...
    encoder.decode_complex64_simd(plain, output);
    print_vector(output);
